...
''' Process DSSC darks with kicked trains to get mean, sigma and masks '''
...
class ProcDarks():
    ...
    def _worker(self, rank, marray, sarray, numarray):
        ...
        with h5py.File(fname, 'r') as f:
            ...
            # indices to process
            inds = np.where(tid % 10 == 9)[0]
            
            num_chunks = int(np.ceil(inds.shape[0] / CHUNK_SIZE))
            for chunk in range(num_chunks):
                st, en = chunk*CHUNK_SIZE, (chunk+1)*CHUNK_SIZE
                chunk_inds = inds[st:en]
                 
                cells  = self.cell_mask[cid[chunk_inds]]
                frames = np.squeeze(dset[chunk_inds]).astype('f4')
...
def main():
    ...
    if args.out_fname is None:
        args.out_fname = PREFIX + 'dark_kicked_trains/r%.4d_dark.h5'%args.run
